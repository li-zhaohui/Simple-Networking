...
__GPL__ = """
   ...
   ** MGcrack is an extension of Sipvicious by MGKnight <mgknight@ymail.com> **
...
from svcrack import ASipOfRedWine as gocrack
...
def startgocrack(host,username,crackmode,crackargs,extension,port,externalip,maximumtime,localport,domain):
	try:
		cracker = gocrack(host,username=username,crackmode=crackmode,crackargs=crackargs,extension=extension,port=port,externalip=externalip,maxlastrecvtime=maximumtime,localport=localport,domain=domain)
	except Exception as e:
		print "WTF, error initiating gocrack on extension ",extension, " :_", e
	try:
		cracker.start()
	except Exception as e:
		print "WTF, error starting gocrack on extension ",extension, " :_", e
	return cracker.resultpasswd
...
#   SIP response codes, also mapped to ISDN Q.931 disconnect causes.
	...
	UNAVAILABLE = 'SIP/2.0 480 ' #Temporarily unavailable
		...
		if firstline != self.BADUSER:
			...
			if self.type == "blind":
				try:
					...
					crackmode = 1
					crackargs = extension
					self.resultpasswd = startgocrack(self.dsthost,Cusername,crackmode,crackargs,extension,self.dstport,self.externalip,self.maxlastrecvtime,self.localport,self.domain)
					
					lenres = len(self.resultpasswd)
					if lenres > 0:
						for k in self.resultpasswd.keys():
							print "Found: %s:%d,%s,%s" % (self.dsthost,self.dstport, extension, self.resultpasswd[k])
							saveresult("%s:%d,%s,%s\r\n" % (self.dsthost,self.dstport, extension, self.resultpasswd[k]))
							found.append("%s:%d,%s,%s\r\n" % (self.dsthost,self.dstport, extension, self.resultpasswd[k]))
						self.nomore = True
						return 
				except Exception as e:
					print "WTF, error in gocrack:_", e
			...
			or buff.startswith(self.AUTHREQ):
				...
				if self.sessionpath is not None and self.dbsyncs:
					self.resultauth.sync()
				#MG add here; start gocrack on the same socket
				try:
					Cusername = extension
					crackmode = 1
					crackargs = extension
				...
				try:
					...
					crackmode=2
					try:
						dictionary = open("pwd.dict",'r')
					except IOError:
						logging.error("could not open %s" % dictionary)
					self.resultpasswd = startgocrack(self.dsthost,Cusername,crackmode,crackargs,extension,self.dstport,self.externalip,self.maxlastrecvtime,self.localport,self.domain)
					lenres = len(self.resultpasswd)
					if lenres > 0:
						for k in self.resultpasswd.keys():
				...
				if self.sessionpath is not None and self.dbsyncs:
					self.resultauth.sync()
				#MG add here; finish ip cracking with this extension
				print "Found: %s:%d,%s,<noauth>\r\n" % (self.dsthost, self.dstport, extension)
				saveresult("%s:%d,%s,<noauth>\r\n" % (self.dsthost, self.dstport, extension))
				found.append("%s:%d,%s,<noauth>\r\n" % (self.dsthost, self.dstport, extension))
				self.nomore = True
				return
				...
				try:
					...
					if lenres > 0:
						for k in self.resultpasswd.keys():
							print "Found: %s,%s,%s" % (Chost, extension, self.resultpasswd[k])
							saveresult("%s,%s,%s\r\n" % (Chost, extension, self.resultpasswd[k]))
							found.append("%s:%d,%s,%s\r\n" % (self.dsthost,self.dstport, extension, self.resultpasswd[k]))
						self.nomore = True
						return 
				except Exception as e:
	...
	def start(self):
		...
		try:
			...
			self.log.error("socket error,mysendto,: %s" % err)
			return
		...
		try:
			while 1:
				try:
					...
					self.log.error("socket error,recvfrom: %s" % err)
					return
				if buff.startswith(self.TRYING) \
					...
					self.log.error("SIP server replied with an authentication request for an unknown extension. setting ip type to blind.")
					self.type = "blind"
					self.BADUSER = "nobaduser"
					break
					#return
				elif buff.startswith(self.OKEY) and self.initialcheck:
					self.log.error("SIP server replied with '200 OK' for an unknown extension. Stopping")
					return
				else:
			...
			self.log.error("socket error,recvfrom2: %s" % err)
			return
		...
		while 1:
			...
			if r:
				...
				try:
					...
					self.log.error("socket error,z,: %s" % err)
					break
