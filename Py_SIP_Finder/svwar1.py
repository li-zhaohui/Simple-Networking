	...
	def getResponse(self):
		...
		if firstline != self.BADUSER:
				...
				try:
					...
					if lenres > 0:
						for k in self.resultpasswd.keys():
							print "Found: %s,%s,%s" % (self.dsthost, extension, self.resultpasswd[k])
							saveresult("%s,%s,%s\r\n" % (self.dsthost, extension, self.resultpasswd[k]))
							found.append("%s,%s,%s\r\n" % (self.dsthost, extension, self.resultpasswd[k]))
						self.nomore = True
						return 
				except Exception as e:
			...
			or buff.startswith(self.AUTHREQ):
				...
				print "Found: %s,%s,<noauth>\r\n" % (self.dsthost, extension)
				saveresult("%s,%s,<noauth>\r\n" % (self.dsthost, extension))
				found.append("%s,%s,<noauth>\r\n" % (self.dsthost, extension))
				self.nomore = True
				return
